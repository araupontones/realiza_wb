...
sgr_clean <- import(infile_sgr) %>% rename(Grupo = Grupos_fixos)
...
emprendedoras_dashboard <- do.call(plyr::rbind.fill,emprendedoras)%>%
  ...
  mutate(
    ...
    Tipo = case_when(grepl("coaching", Modulo) ~ "Coaching",
...
sgr_stats <-all_divs %>%
  count_asistencias(responsible = "Grupo", name_actividade = "Tipo", "Cidade") %>%
  relocate(Cidade)
...
sgr_div <- sgr_stats %>%
  ...
  pivot_wider(id_cols = c(Cidade, Grupo, Emprendedora),
              names_from = Tipo,
              values_from = div)
  View(sgr_div)          
